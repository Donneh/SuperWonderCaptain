...
def highscore_popup():
    ...
    tree.pack(fill="both")
    Label(mt, text="highscore", height=20, width=70).pack()
    Button(mt, text="Sluit venster", command=mt.destroy).place(x=350, y=120)
    mt.resizable(0, 0)
...
def derde_scherm():
    ...
    # Creatie van het starten van het spel en het spelen
        ...
        highscore_button = Button(label_achtergrond, text="highscore", command=highscore_popup).place(x=500, y=10)
...
def hintophalen(superheld):
    try:
        ...
        print("Er zijn geen gegevens aanwezig")
